...
make_treatment_plot <- function(data, emms, letters, response_var, y_label, label_y = NULL) {
...
Combined effects of 2-yr (2022-2023) pika and *S. chamaejasme* removal treatments on neutral detergent fibre (NDF) of total forage in the field manipulated experiment. The average values of each variable in the two years were used for statistical analysis, providing a single data point for each variable in each 150m × 150 m plot. Error bars represent +/- SE.
:::
...
Combined effects of 2-yr (2022-2023) pika and poisonous plant removal treatments on foraging efficiency of yaks in the field manipulated experiments. (A) bite rates and (B) bites per step of yaks for forbs, respectively. The average values of each variable in the two years were used for statistical analysis, providing a single data point for each variable in each 150 m × 150 m plot. Error bars represent +/- SE.
:::
...
yak_performance_summary_table <- bind_rows(
  ...
  forage_quality_models$ee$model_summary |> mutate(model = 'neutral detergent fiber %'))  |>
  mutate(term = case_when(str_detect(term,'sd__') ~ group,
    TRUE ~ term),
    effect = ifelse(effect == 'ran_pars','random','fixed'),
...
yak_performance_contrast_table <- bind_rows(
  ...
  sedges_models$model_contrasts |> mutate(model = 'sedge cover'),
  forbs_models$model_contrasts |> mutate(model = 'forb cover'),
  ...
  forage_quality_models$ndf$model_contrasts |> mutate(model = 'neutral detergent fiber %'))  |>
...
yak_foraging_efficiency_table <- bind_rows(
  ...
  plant_bites_models$forbs$model_summary |> mutate(model = 'total forb bites'),
...
yak_foraging_efficiency_table <- bind_rows(
  ...
  plant_bites_models$forbs$model_contrasts |> mutate(model = 'total forbs bites'),
  ...
  bite_step_models$forbs$model_contrasts |> mutate(model = 'forb bite steps')
  )  |>
  select(contrast,estimate,SE,df,t.ratio,p.value,model)
