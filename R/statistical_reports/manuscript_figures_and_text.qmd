...
sedge_bite_increase <- calc_pct_increase(plant_bites_models$sedges$emms, value_col = 'response')
...
get_family_info <- function(model) {
  ...
  default_result <- list(family = NA_character_, tweedie_power = NA_real_)
  
  # Wrap entire function in tryCatch to ensure we always return proper structure
  tryCatch({
    if (is.null(model)) return(default_result)
    ...
    if (is.null(actual_model)) return(default_result)
  ...
  if (!is.na(family_name) && grepl("tweedie", family_name, ignore.case = TRUE)) {
    ...
    if (inherits(actual_model, "glmmTMB")) {
      ...
      tweedie_power <- tryCatch({
        ...
        else if (!is.null(actual_model$modelInfo$family$par)) {
          actual_model$modelInfo$family$par
        }
        # Method 3: from family link parameters
        else if (!is.null(actual_model$fit$par)) {
          # The Tweedie power is stored in the dispersion parameters
          disp_par <- actual_model$fit$par
          if (length(disp_par) >= 2) {
            disp_par[2]  # Tweedie power is typically the 2nd parameter
          } else {
            NA_real_
          }
        }
    ...
    warning(paste("Error in get_family_info:", e$message))
...
# Debug: Check family_info structure for problematic models
for (i in seq_along(model_info$family_info)) {
  fi <- model_info$family_info[[i]]
  if (!is.list(fi) || !all(c("family", "tweedie_power") %in% names(fi))) {
    warning(paste("Model", model_info$model_name[i], "has invalid family_info:",
                  "is.list =", is.list(fi),
                  "| type =", typeof(fi),
                  "| class =", paste(class(fi), collapse = ", "),
                  "| names =", paste(names(fi), collapse = ", ")))
  }
}
...
model_info <- model_info |>
  mutate(
    family = map_chr(family_info, function(x) {
      if (is.list(x) && "family" %in% names(x)) {
        return(x$family)
      } else {
        return(NA_character_)
      }
    }),
    tweedie_power = map_dbl(family_info, function(x) {
      if (is.list(x) && "tweedie_power" %in% names(x)) {
        return(x$tweedie_power)
      } else {
        return(NA_real_)
      }
    }),
...
model_overview <- model_info |>
  mutate(
    ...
    random_effects = case_when(
      ...
      # Bite-steps ratio models don't have year (data limitation)
      grepl("bite_steps", model_name) ~ gsub(", year", "", random_effects, fixed = TRUE),
      TRUE ~ random_effects
    ),
    ...
    sample_size = case_when(
      ...
      # Bite-steps ratio models don't have year in the data
      grepl("bite_steps", model_name) ~ glue("n = {n_experiment_plots} plots"),
