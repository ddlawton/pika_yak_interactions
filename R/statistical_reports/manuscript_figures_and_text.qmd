...
Food quantity and quality are key drivers of individual performance and population dynamics of large herbivores^30-32^. In the presence of *S. chamaejasme* forbs, pika doubled the cover of yak’s most favored grasses ( *@fig-diet-selection* C, @supptbl-yak-performance-model-summary , @supptbl-yak-performance-model-contrast)and increased crude protein content of total forage by approximately 14% (@fig-yak-performance D, @supptbl-yak-performance-model-summary, @supptbl-yak-performance-model-contrast), suggesting that pika facilitate yak by increasing the quantity and quality of forage plants. Acid detergent fibre and ether extract were also sigificantly different between the plots (@fig-yak-performance E & F, @supptbl-yak-performance-model-summary, @supptbl-yak-performance-model-contrast). The increases in grass abundance and forage quality were likely driven by the decline in poisonous plant abundance induced by pika (@fig-yak-performance B), which released inter-specific competition for shared resources (e.g., light, soil moisture and nutrients) and allowed their growth and expansion^27-28^. In the absence of *S. chamaejasme* forbs, however, the positive effects of pika on food resources and weight gain of yak disappeared (@fig-yak-performance A,C,D). Pika and poisonous plants impacted sedge cover similiarly like grass cover (@suppfig-forb-sedge-cover A) and there was a more complicated interaction with forb cover (@suppfig-forb-sedge-cover B). There was no impact of pika and poisonous plant on neutrl detergent fibre (@suppfig-forb-sedge-cover C).


```{r bite increase}

total_bite_increase <- plant_bites_models$total$emms |>
  filter(posion_plant_treatment == "S. chamaejasme") |>
  summarise(
    pct_increase = (emmean[pika_treatment == "pika"] -
                    emmean[pika_treatment == "no pika"]) /
                    emmean[pika_treatment == "no pika"] * 100
  ) |> pull()


sedge_bite_increase <- plant_bites_models$sedges$emms |>
  filter(posion_plant_treatment == "S. chamaejasme") |>
  summarise(
    pct_increase = (response[pika_treatment == "pika"] -
                    response[pika_treatment == "no pika"]) /
                    response[pika_treatment == "no pika"] * 100
  ) |> pull()

grass_bite_steps <- bite_step_models$grasses$emms  |>
  filter(posion_plant_treatment == "S. chamaejasme") |>
  summarise(
    pct_increase = (response[pika_treatment == "pika"] -
                    response[pika_treatment == "no pika"]) /
...
sedge_bite_steps <- bite_step_models$sedges$emms  |>
  filter(posion_plant_treatment == "S. chamaejasme") |>
  summarise(
    pct_increase = (response[pika_treatment == "pika"] -
                    response[pika_treatment == "no pika"]) /
