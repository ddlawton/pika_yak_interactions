...
make_treatment_plot <- function(data, emms, letters, response_var, y_label, label_y = NULL) {
  ...
  ggplot(data, aes(x = pika_treatment, y = .data[[response_var]], fill = posion_plant_treatment)) +
...
plot_plant_bites <- function(plant_name, y_label, label_y = NULL) {
  ...
  ggplot(data, aes(x = pika_treatment, y = forage_efficiency, fill = posion_plant_treatment)) +
    geom_boxplot(position = position_dodge(width = 1), color = "black", outlier.shape = NA) +
    geom_text(
      data = letters,
      aes(x = pika_treatment, y = label_y, label = .group, group = posion_plant_treatment),
...
panel_c <- plot_bite_steps("sedges", "Sedges bite/steps", label_y = 1)
panel_d <- plot_bite_steps("grasses", "Grass bite/steps")
...
ndf_plot <- make_treatment_plot(
  ...
  letters = forage_quality_models$ndf$letters,
  response_var = "forage_quality",
  y_label = "NDF (%)",
  label_y = 70
)
...
# Generate bite panels
panel_a <- plot_plant_bites("forbs", "Forb bites", label_y = 200)

# Generate bite/step panels
panel_b <- plot_bite_steps("forbs", "Forbs bite/steps", label_y = 0.5)
