...
We firstly carried out a set of field observational experiments to investigate the diet selections of pika and yak, and the associations among pika abundance, S. chamaejasme abundance, and yak activities under unmanipulated field conditions. Consistent with previous studies, we found that pika and yak have a distinguished diet preferences: yaks preferred grasses and sedge, whereas pika preferred the poisonous S. chamaejasme forbs (@fig-2 A, B). Moreover, we found that the S. chamaejasme abundance was negatively associated with active pika burrow abundance (R^2 = 0.731, P = 0.002, @fig-2 C), whereas the S. chamaejasme abundance was negatively associated with yak foraging activities, which indicated by dung density (R^2 = 0.412, P < 0.001, @fig-2 D).

Given these results, we then conducted an in-situ manipulative field experiment to explicitly examine the interactive effects of pika and *S. chamaejasme* forbs on weight gain, foraging quantity and quality, and grazing behaviors of yak by using fencing enclosure plots. We found an opposite effect of poisonous plants and pika presence on yak growth performance. Yaks had a slower weight gain in the no pika, *S. chamaejasme* plots (@fig-3 A) likely due to Pika feeding preferences (@fig-2 A).




We found an opposing effect of poisonous plants and pika on yak growth performance; *S. chamaejasme* plot significantly reduced, whereas pika increased weight gain of yak by 15% (*S. chamaejasme*, F1,3 =16.36, P = 0.027) and 27% (pika, F1,3 =87.36, P = 0.03), respectively. 
...
panel_a_weight_gain <- make_treatment_plot(
  data    = weight_gain,
...
panel_b_cp <- make_treatment_plot(
...
panel_c_adf <- make_treatment_plot(
...
panel_d_ee <- make_treatment_plot(
...
panel_e_grass <- make_treatment_plot(
...
panel_f_poison <- make_treatment_plot(
