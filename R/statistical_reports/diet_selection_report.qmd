---
title: "Diet Selection: Exploratory Analysis"
...
This report documents the exploratory analysis of diet selection data that informed the final manuscript analyses. The finalized methods, results, and figures are presented in the **Methods and Results** document.
:::
...
pika_data <- read_csv(here('data/clean/diet_selection/pika_feeding.csv'))
pika_feeding_model_summary <- read_csv(here('data/clean/diet_selection/pika_feeding/model_summary.csv'))
pika_feeding_contrasts <- read_csv(here('data/clean/diet_selection/pika_feeding/posthoc_contrasts.csv'))
pika_feeding_emms <- read_csv(here('data/clean/diet_selection/pika_feeding/emms.csv'))
pika_feeding_letters <- read_csv(here('data/clean/diet_selection/pika_feeding/posthoc_letters.csv'))

yak_data <- read_csv(here('data/clean/diet_selection/yak_grazing.csv'))
yak_feeding_model_summary <- read_csv(here('data/clean/diet_selection/yak_grazing/model_summary.csv'))
yak_feeding_contrasts <- read_csv(here('data/clean/diet_selection/yak_grazing/posthoc_contrasts.csv'))
yak_feeding_emms <- read_csv(here('data/clean/diet_selection/yak_grazing/emms.csv'))
yak_feeding_letters <- read_csv(here('data/clean/diet_selection/yak_grazing/posthoc_letters.csv'))

dung_burrow_by_plot <- read_csv(here('data/clean/diet_selection/dung_burrow_by_plot.csv')) |>
    mutate(plot = factor(plot))

# Load pre-fitted GAM model from saved .qs file
burrow_model <- qs2::qs_read(here('data/models/burrow_cover_gam.qs'))

# Extract GAM smooth estimates for plotting
gam_ests <- gratia::smooth_estimates(burrow_model)
gam_ests$adj_est <- gam_ests$.estimate + coef(burrow_model)[1]

summary(burrow_model)
```
