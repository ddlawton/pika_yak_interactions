...
**Diet Selection** - Pika feeding - Yak grazing - Dung density - Pika burrow density

**Experiment: Plant Cover** - Forbs - Grasses - *S. chamaejasme* - Sedges

**Experiment: Foraging Efficiency** - Yak bite/step ratios - Yak bites by type - Yak total steps - Yak weight gain

**Experiment: Foraging Quality** - ADF - CP - EE - NDF
...
The following figure (@fig-diet_selection_figure) shows pika feedind frequency (A) and yak grazig frequency (B) by every pika and posion plant treatment with sigificance shown with letters. It also shows the relationship between pika burrow density (C) and yak dung density (D) by *S. chamaejasme* cover.
...
Yak grazing overall was significantly reduced by S. chamaejasme treatment, with 131 fewer bites on average compared to control plots without the poisonous plant. Pika presence alone did not significantly influence total bites. However, the strong positive interaction term (+119.5) indicates that pika presence substantially offsets the negative effect of S. chamaejasme, restoring grazing rates close to control levels. Contrasts confirm that while S. chamaejasme sharply reduces total bites, this effect is largely absent when pika are present.
...
Yak movement (total steps) was significantly higher in S. chamaejasme plots, with an average increase of 113 steps compared to control plots without the poisonous plant (@tbl-total-step-model-summary). Pika presence alone did not significantly affect step counts. However, the strong negative interaction term (−136.1, @tbl-tbl-total-step-model-summary) indicates that when pika were present, the S. chamaejasme-induced increase in movement was eliminated. Contrasts show that while S. chamaejasme alone led to more steps, this effect was absent or reversed in the presence of pika, suggesting pika dampen the plant’s impact on yak movement patterns (@total-step-model-contrasts).
...
The poisonous plant S. chamaejasme substantially reduced yak weight gain, with a mean loss of 0.108 kg/day compared to control plots without the plant while pika presence alone had no significant effect on weight gain (@tbl-weight-gain-model-summary). However, the strong positive interaction term (+0.132 kg/day) indicates that pika presence largely neutralizes the negative effect of S. chamaejasme, resulting in weight gain similar to control conditions. Contrasts show that while S. chamaejasme sharply reduces weight gain in the absence of pika, this reduction disappears when pika are present, suggesting a facilitative role of pika in mitigating plant-induced losses (@tbl-weight-gain-model-contrasts).
...
# Reusable function
plot_forage_quality <- function(quality_metric, y_label, label_y = NULL) {
  data <- forage_quality |> filter(plant == quality_metric)
  emms <- forag_quality_models[[quality_metric]]$emms
  letters <- forag_quality_models[[quality_metric]]$letters
...
ndf_plot   <- plot_forage_quality("ndf", "ndf (%)",label_y=68)
...
ADF content was substantially reduced in pika removal plots, with the largest drop occurring when poisonous plants were also removed. When pikas were present, ADF levels were highest, especially in the presence of poisonous plants. Poisonous plant removal alone had a moderate effect on lowering ADF, but pika removal was the dominant driver. Lower ADF values under removal treatments point to improved forage digestibility.
...
Crude protein content was significantly higher in plots with pika removal, especially when poisonous plants were also removed. The combination of both treatments led to the greatest protein levels, suggesting a synergistic effect. In contrast, plots with pika presence and intact poisonous plants had the lowest crude protein content. This pattern indicates that both pikas and poisonous plants contribute to reduced forage nutritional value.
...
NDF levels were lowest in pika removal plots, particularly when poisonous plants were also removed. Higher NDF values were observed when pikas were present, reflecting lower forage digestibility under these conditions. Poisonous plant removal alone reduced NDF, but the effect was strongest when combined with pika removal. These results suggest both pikas and poisonous plants increase fiber content, reducing forage quality.
...
```{r}
#| label: tbl-ndf-model-contrasts
#| tbl-cap: "Estimated marginal means contrasts for grass cover by treatment group."
#| echo: false
#| message: false
#| warning: false

forag_quality_models$ndf$model_contrasts |>
  kable(format = "latex", booktabs = TRUE, digits = 3) |>
  kable_styling(latex_options = c("striped"))
```